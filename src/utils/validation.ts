...
export class ValidationService {
  ...
  static validateConfig(config: GraphifyConfig): {
    ...
    try {
      ...
      if (config.startDate) {
        ...
        } else if (start > DateTime.now() && config.preventFutureCommits !== false) {
      ...
      if (config.endDate) {
        ...
        } else if (end > DateTime.now() && config.preventFutureCommits !== false) {
      ...
      errors.push(`Error validating configuration: ${error instanceof Error ? error.message : String(error)}`);
  ...
  static analyzeDateDistribution(dates: string[]): {
    ...
    try {
      ...
      ErrorHandler.getInstance().handle(
        new GraphifyError(
          `Error analyzing date distribution: ${error instanceof Error ? error.message : String(error)}`,
      ...
      return {
        hasIssues: true,
        suggestions: ['Error analyzing date distribution.'],
        analytics: { totalCommits: dates.length },
      };
    }
  }
  ...
  static async validateRepository(repoPath: string): Promise<{
    ...
    try {
      ...
      ErrorHandler.getInstance().handle(
        new GraphifyError(
          `Repository validation error: ${errorMessage}`,
          ErrorCategory.VALIDATION,
          error instanceof Error ? error : undefined
        )
      );
