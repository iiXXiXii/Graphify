  ...
  importCommand
    ...
    .action(async (options) => {
      try {
        ...
        const pattern = await importPattern(options.file, options.format);
        ...
        if (options.output) {
          await savePattern(pattern, options.output);
        } else {
          ...
          if (shouldSave) {
            ...
            await savePattern(pattern, outputPath);
          }
        }
      } catch (error) {
...
async function importPattern(filePath: string, format: string): Promise<number[][]> {
  try {
...
async function importFromJson(content: string): Promise<number[][]> {
  try {
    ...
    if (validationIssues.length > 0) {
      ...
      validationIssues.forEach(issue => console.log(chalk.yellow(`- ${issue}`)));
...
async function importFromCsv(content: string): Promise<number[][]> {
  try {
    // Parse CSV content
...
function showPatternInfo(pattern: number[][]): void {
  ...
  console.log(`Active cells: ${activeCells} (${Math.round(activeCells / (rows * cols) * 100)}%)`);
...
function printPatternPreview(pattern: number[][]): void {
  ...
  if (pattern.length > MAX_PREVIEW_SIZE || pattern[0]?.length > MAX_PREVIEW_SIZE) {
    console.log(chalk.dim('(Pattern preview truncated due to size)'));
  }
}

async function savePattern(pattern: number[][], outputPath: string): Promise<void> {
  try {
